...
function verifyChoices() {
	...
	if (round.answer.length === currentGame.recallCount) {
		round.correct = reduce(round.answer, function (result, a, i) {
			return round.stage[i] === a && result;
		});
		if (round.correct) {
			showCorrectMessage();
...
function saveScore(score) {
	...
	if (!existingScoreData.content.topScore ||
		latestScoreData.score > existingScoreData.content.topScore.score) {
...
if (useOpenSansEmoji || (navigator.userAgent.match(/windows/i)) || (navigator.userAgent.match(/linux/i) && !navigator.userAgent.match(/android/i))) {
